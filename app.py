...
Features:
  ...
  - Vision-Language (VL) support for diagram extraction
  - .docx export matching user's template format

Stack: ChromaDB + SentenceTransformers + llama.cpp (100% offline)
"""
...
# VL Processor (optional — works without VL model too)
try:
    from vl_extract import (
        extract_and_describe_images, format_vl_descriptions_for_prompt,
        extract_images
    )
    VL_AVAILABLE = True
    print("[VL] Vision-Language module loaded")
except ImportError:
    VL_AVAILABLE = False
    print("[VL] vl_extract.py not found — VL features disabled (text-only mode)")

# HTML Export
try:
    from html_export import markdown_to_html
    HTML_EXPORT = True
    print("[HTML] HTML export module loaded")
except ImportError:
    HTML_EXPORT = False
    print("[HTML] html_export.py not found — HTML export disabled")
...
# ============================================================
# VL (VISION-LANGUAGE) IMAGE EXTRACTION
# ============================================================
# Extracts images/diagrams from PDFs and uses a VL model
# (LLaVA, MiniCPM-V, etc.) via llama.cpp to describe them.
# These descriptions are added to the RAG context.
#
# Requires: llama-llava-cli or llama-minicpmv-cli in llama.cpp build
# Model: Download a GGUF VL model (e.g., llava-v1.5-7b-q4_k_m.gguf)
#
# Set VL_MODEL_PATH and VL_PROJECTOR_PATH below, or set to None to disable.
# ============================================================

VL_ENABLED = os.environ.get("VL_ENABLED", "false").lower() == "true"
VL_MODEL_PATH = os.environ.get("VL_MODEL_PATH", None)
VL_PROJECTOR_PATH = os.environ.get("VL_PROJECTOR_PATH", None)
VL_CLI_PATH = os.environ.get("VL_CLI_PATH", None)  # path to llama-llava-cli or llama-minicpmv-cli
VL_IMAGE_DIR = "./vl_images"
os.makedirs(VL_IMAGE_DIR, exist_ok=True)

# Minimum image size to process (skip tiny icons/logos)
VL_MIN_WIDTH = 200
VL_MIN_HEIGHT = 200


def extract_images_from_pdf(filepath, max_images=10):
    """Extract meaningful images/diagrams from a PDF"""
    images = []
    try:
        doc = fitz.open(filepath)
        img_count = 0
        ...
        for page_num in range(len(doc)):
            ...
            for img_idx, img_info in enumerate(image_list):
                ...
                try:
                    ...
                    if base_image:
                        ...
                        if width >= VL_MIN_WIDTH and height >= VL_MIN_HEIGHT:
                            img_path = os.path.join(
                                VL_IMAGE_DIR,
                                f"{Path(filepath).stem}_p{page_num}_i{img_idx}.{base_image['ext']}"
                            )
            ...
            # Method 2: Render page as image (catches diagrams drawn with vector graphics)
            # Only do this for pages that have few text blocks (likely diagram pages)
            text_blocks = page.get_text("blocks")
            text_content = " ".join(b[4] for b in text_blocks if b[6] == 0)  # type 0 = text
            if len(text_content) < 100 and len(text_blocks) > 0:
                # This page is mostly visual — render it
                if img_count < max_images:
                    pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))  # 2x zoom
                    img_path = os.path.join(
                        VL_IMAGE_DIR,
                        f"{Path(filepath).stem}_p{page_num}_full.png"
                    )
                    pix.save(img_path)
                    images.append({
                        "path": img_path,
        ...
        print(f"[VL] Error extracting images: {e}")

    return images


def describe_image_with_vl(image_path, context=""):
    """Use VL model to describe an image/diagram"""

    # Method 1: Use llama.cpp CLI (works offline)
    if VL_CLI_PATH and VL_MODEL_PATH and VL_PROJECTOR_PATH:
        import subprocess
        prompt = (
            "Describe this technical diagram or figure in detail. "
            "Identify all components, connections, interfaces, data flows, "
            "and any text labels. If it's a block diagram, list all blocks "
            "and their interconnections. If it's a table, extract all data."
        )
        if context:
            prompt += f"\nContext: This is from an HRS/SRS document about: {context}"

        cmd = [
            VL_CLI_PATH,
            "-m", VL_MODEL_PATH,
            "--mmproj", VL_PROJECTOR_PATH,
            "--image", image_path,
            "-p", prompt,
            "-ngl", "99",
            "--temp", "0.1",
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"[VL] CLI error: {result.stderr[:200]}")
                return None
        except subprocess.TimeoutExpired:
            print("[VL] Timeout processing image")
            return None
        except Exception as e:
            print(f"[VL] Error: {e}")
            return None

    # Method 2: Use llama.cpp server with multimodal endpoint (if available)
    elif VL_ENABLED:
        try:
            import base64
            ...
            payload = {
                "prompt": "[INST] Describe this technical diagram in detail. List all components, connections, and labels. [/INST]",
            ...
            print(f"[VL] Server multimodal error: {e}")

    return None


def extract_visual_context(filepath, project_context=""):
    """Full pipeline: extract images → describe with VL → return text descriptions"""
    if not VL_ENABLED and not VL_CLI_PATH:
        return []

    ext = Path(filepath).suffix.lower()
    if ext != ".pdf":
        return []  # Only PDF image extraction supported currently

    print(f"[VL] Extracting images from {Path(filepath).name}...")
    images = extract_images_from_pdf(filepath)
    print(f"[VL] Found {len(images)} images/diagrams")

    descriptions = []
    for i, img in enumerate(images):
        print(f"[VL] Describing image {i+1}/{len(images)}: {Path(img['path']).name} "
              f"({img['width']}x{img['height']}, page {img['page']})")
        desc = describe_image_with_vl(img["path"], project_context)
        if desc:
            descriptions.append({
                "page": img["page"],
                "type": img["type"],
            ...
            print(f"[VL] ✗ Could not describe image")
...
def _query_llamacpp(prompt, system_prompt, temperature):
    ...
    # Trim prompt if too large (keep under ~3500 tokens for 4096 context)
    max_prompt_chars = 14000  # ~3500 tokens
    if len(full_prompt) > max_prompt_chars:
        print(f"[LLM] WARNING: Prompt too large ({len(full_prompt)} chars), trimming to {max_prompt_chars}")
        # Keep system prompt + beginning and end of user prompt
        sys_end = full_prompt.find("[/INST]")
        if sys_end == -1:
            ...
            full_prompt = full_prompt[:keep_start] + "\n\n[... middle trimmed for context length ...]\n\n" + full_prompt[-keep_end:]

    print(f"[LLM] Final prompt: {len(full_prompt)} chars (~{len(full_prompt)//4} tokens)")

    payload = {
        "prompt": full_prompt,
        "temperature": temperature,
        "n_predict": 2048,     # Reduced from 4096 — faster, still enough for SRS
        "top_p": 0.9,
        "stop": ["</s>", "[INST]"],
        "stream": False,
        "cache_prompt": True,  # Reuse cached prompt for repeated queries
    }
    resp = requests.post(
        f"{LLAMACPP_BASE_URL}/completion",
        json=payload, timeout=1800  # 30 min timeout
    )
    resp.raise_for_status()
    data = resp.json()

    # Print timing info from llama.cpp
    if "timings" in data:
        t = data["timings"]
        print(f"[LLM] Prompt eval: {t.get('prompt_n', '?')} tokens in {t.get('prompt_ms', 0)/1000:.1f}s "
              f"({t.get('prompt_per_second', 0):.1f} t/s)")
        print(f"[LLM] Generation:  {t.get('predicted_n', '?')} tokens in {t.get('predicted_ms', 0)/1000:.1f}s "
              f"({t.get('predicted_per_second', 0):.1f} t/s)")
...
def build_context(project_name, requirements_text, doc_type="srs"):
    ...
    # Reduced n_results for faster prompt processing on Jetson
    template_chunks = retrieve_context(
        f"{doc_type} template document structure format sections",
        collection_names=[template_coll], n_results=4  # was 8
    )
    reference_chunks = retrieve_context(
        f"{project_name} {requirements_text[:200]}",
        collection_names=["reference_docs"], n_results=3  # was 5
    )
    req_chunks = retrieve_context(
        requirements_text[:300],
        collection_names=["requirements"], n_results=3  # was 5
    )

    # Truncate each chunk to max 400 chars to keep prompt small
    MAX_CHUNK = 400
...
def generate_full_srs(project_name, requirements, raw_text, instructions=""):
    ...
    print(f"\n{'='*60}")
    print(f"[SRS GENERATION] Starting for: {project_name}")
    print(f"[SRS GENERATION] Requirements count: {len(requirements)}")
    print(f"[SRS GENERATION] Raw text length: {len(raw_text)} chars")
    print(f"{'='*60}")

    print("[STEP 1/4] Retrieving context from knowledge base...")
    t0 = time.time()
    context = build_context(project_name, raw_text, "srs")
    t1 = time.time()
    print(f"[STEP 1/4] ✓ Context retrieved in {t1-t0:.1f}s")
    print(f"  - Template chunks: {len(context['templates'])} chars")
    print(f"  - Reference chunks: {len(context['references'])} chars")
    print(f"  - Stored requirements: {len(context['stored_reqs'])} chars")

    print("[STEP 2/4] Formatting requirements...")
    formatted_reqs = format_requirements_for_prompt(requirements)
    print(f"[STEP 2/4] ✓ Formatted {len(requirements)} requirements")

    print("[STEP 3/4] Building prompt...")
    prompt = f"""
...
"""
    ...
    print(f"[STEP 3/4] ✓ Prompt built (~{prompt_tokens} words, ~{len(prompt)} chars)")

    print(f"[STEP 4/4] Sending to LLM ({detect_backend()})...")
    print(f"[STEP 4/4] ⏳ This will take 5-15 minutes on Mistral 7B...")
    print(f"[STEP 4/4]    Watch llama.cpp terminal for token generation progress")
    t2 = time.time()
    result = query_llm(prompt, system_prompt=SRS_SYSTEM_PROMPT, temperature=0.3)
    t3 = time.time()

    if result.startswith("ERROR"):
        print(f"[STEP 4/4] ✗ LLM Error: {result[:100]}")
    else:
        result_words = len(result.split())
        print(f"[STEP 4/4] ✓ Generation complete in {t3-t2:.1f}s")
        print(f"  - Output: {result_words} words, {len(result)} chars")
        print(f"  - Speed: ~{result_words / max(t3-t2, 1) * 60:.0f} words/min")

    print(f"[SRS GENERATION] Total time: {t3-t0:.1f}s ({(t3-t0)/60:.1f} min)")
    print(f"{'='*60}\n")
    return result
...
def generate_full_sdd(project_name, srs_content, instructions=""):
    ...
    print(f"\n{'='*60}")
    print(f"[SDD GENERATION] Starting for: {project_name}")
    print(f"[SDD GENERATION] SRS content length: {len(srs_content)} chars")
    print(f"{'='*60}")

    print("[STEP 1/3] Retrieving SDD context from knowledge base...")
    t0 = time.time()
    context = build_context(project_name, srs_content[:1000], "sdd")
    print(f"[STEP 1/3] ✓ Context retrieved in {time.time()-t0:.1f}s")

    print("[STEP 2/3] Building prompt...")
    prompt = f"""
PROJECT: {project_name}
DATE: {datetime.now().strftime('%Y-%m-%d')}
...
"""
    print(f"[STEP 2/3] ✓ Prompt: ~{len(prompt.split())} words")

    print(f"[STEP 3/3] Sending to LLM ({detect_backend()})...")
    print(f"[STEP 3/3] ⏳ Generating SDD (5-15 minutes)...")
    t2 = time.time()
    result = query_llm(prompt, system_prompt=SDD_SYSTEM_PROMPT, temperature=0.3)
    t3 = time.time()

    if result.startswith("ERROR"):
        print(f"[STEP 3/3] ✗ Error: {result[:100]}")
    else:
        print(f"[STEP 3/3] ✓ Done in {t3-t2:.1f}s ({len(result.split())} words)")

    print(f"[SDD GENERATION] Total: {t3-t0:.1f}s ({(t3-t0)/60:.1f} min)")
    print(f"{'='*60}\n")
    return result
...
def api_status():
    ...
    vl_status = check_vl_available() if VL_AVAILABLE else {"available": False, "reason": "module not loaded"}
    return jsonify({
        "ollama": llm_status["running"],
        "backend": llm_status["backend"],
        "model": llm_status["model"] or OLLAMA_MODEL,
        "vl_available": vl_status.get("available", False),
        "vl_info": vl_status,
        "collections": {
...
def api_extract_requirements():
    """Upload HRS file and extract requirements + visual content"""
    visual_descriptions = []
    ...
    if "file" in request.files:
        ...
        # VL: Extract and describe images/diagrams
        if VL_AVAILABLE:
            try:
                VL_URL = os.environ.get("VL_URL", "http://localhost:8081")
                visual_descriptions = extract_and_describe_images(
                    filepath, llamacpp_url=VL_URL, max_images=6
                )
                if visual_descriptions:
                    vl_text = format_vl_descriptions_for_prompt(visual_descriptions)
                    raw_text += vl_text
                    print(f"[VL] Added {len(visual_descriptions)} diagram descriptions to context")
            except Exception as e:
                print(f"[VL] Error (non-fatal): {e}")
    ...
    return jsonify({
        ...
        "visual_descriptions": len(visual_descriptions),
    })
...
def api_generate():
    ...
    try:
        ...
        # Save as HTML (primary output)
        html_filename = f"{project_name.replace(' ', '_')}_{doc_type.upper()}.html"
        html_path = os.path.join(OUTPUT_DIR, html_filename)
        if HTML_EXPORT:
            html_content = markdown_to_html(
                content,
                title=f"{project_name} — {doc_type.upper()}",
                project_name=project_name,
                doc_type=doc_type.upper()
            )
            with open(html_path, "w", encoding="utf-8") as f:
                f.write(html_content)
            print(f"[OUTPUT] Saved HTML: {html_filename}")
        else:
            html_filename = None

        # Also save .docx as backup
        ...
        if doc_type == "srs":
            ...
            project_data["srs_html"] = html_filename
        else:
            project_data["sdd_content"] = content
            project_data["sdd_docx"] = docx_filename
            project_data["sdd_html"] = html_filename
...
def api_generate_sdd_pipeline():
    ...
    # Save HTML
    html_filename = f"{project_data['project_name'].replace(' ', '_')}_SDD.html"
    html_path = os.path.join(OUTPUT_DIR, html_filename)
    if HTML_EXPORT:
        html_content = markdown_to_html(
            sdd_content,
            title=f"{project_data['project_name']} — SDD",
...
def api_preview(filename):
    """Serve HTML file for in-browser preview"""
    filepath = os.path.join(OUTPUT_DIR, secure_filename(filename))
